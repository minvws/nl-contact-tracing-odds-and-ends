...
\def\title{Technical Debt Primer\xspace}
\def\subject{Webweaving Primer on Technical Debt\xspace}
\def\opensource{no} % yes or no (lowercase)
\def\hidepeople{no} % yes or no (lowercase)
\def\status{} 
\def\validityOffer{0}

\input{../../colateral/prospectus-template}
